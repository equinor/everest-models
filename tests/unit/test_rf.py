...
def test_simple_usage(ecl_sum):
    ...
    assert (
        recovery_factor(ecl_sum, start_date="03.01.2000", end_date="07.01.2000") == 0.04
    )
    assert (
        recovery_factor(
            ecl_sum,
...
def test_clamping_behaviour(ecl_sum):
    ...
    assert (
        recovery_factor(ecl_sum, start_date="01.01.1999", end_date="06.01.2000") == 0.05
    )
...
def test_exceptions(ecl_sum):
    ...
    assert "time data '2000.01.03' does not match format '%d.%m.%Y'" in str(
        excinfo.value
    )
