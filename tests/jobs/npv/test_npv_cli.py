...
_WELL_COSTS_N_INPUT_PAIR_ERR_MSG = (
    "-c/--config argument file key 'well_cost' must be paired with "
    "either the key 'wells' or the -i/--input argument."
)
...
def test_npv_main_entry_point(
    copy_testdata_tmpdir, monkeypatch, input_file, wells_input, add_wells_to_config
):
    copy_testdata_tmpdir(TEST_DATA)
    options = {}
    if wells_input == "json":
        options["input"] = parse_file(input_file, Wells)
    else:
        add_wells_to_config(input_file, _CONFIG_FILE)
    options["config"] = parse_file(_CONFIG_FILE, NPVConfig)
    monkeypatch.setattr(
        cli, "build_argument_parser", lambda: MockParser(options=Options(**options))
    )
...
def test_npv_main_entry_point_no_well_costs_error(
    ...
    if wells_input == "json":
        ...
        add_wells_to_config(input_file, _CONFIG_FILE_NO_WELL_COSTS)
    options["config"] = parse_file(_CONFIG_FILE_NO_WELL_COSTS, NPVConfig)
    monkeypatch.setattr(
        cli, "build_argument_parser", lambda: MockParser(options=Options(**options))
    )
    with pytest.raises(SystemExit) as e:
...
def test_npv_main_entry_point_warning_for_default_output(
    copy_testdata_tmpdir, monkeypatch, caplog, wells_input, add_wells_to_config
):
    copy_testdata_tmpdir(TEST_DATA)
    if wells_input == "json":
        args = ["-i", "wells.json", "-c", _CONFIG_FILE]
    else:
        add_wells_to_config("wells.json", _CONFIG_FILE)
        args = ["-c", _CONFIG_FILE]
    monkeypatch.setattr(
        parser,
...
def test_npv_main_entry_point_no_warning_for_non_default_output(
    ...
    cli.main_entry_point([*args, output_arg, "npv_test"])
    assert "Objective names ending in '_0' have been deprecated" not in caplog.text
    assert pathlib.Path("npv_test").exists()


def test_npv_error_both_wells_in_input_and_config(
    copy_testdata_tmpdir, add_wells_to_config, monkeypatch, capsys
):
    copy_testdata_tmpdir(TEST_DATA)
    add_wells_to_config("wells.json", _CONFIG_FILE)
    monkeypatch.setattr(
    ...
    with pytest.raises(SystemExit) as e:
        cli.main_entry_point(["--input", "wells.json", "--config", _CONFIG_FILE])
