...
import pytest
from jobs.npv_.helper import _CONFIG_FILE, assert_written_npv
...
def test_base_case_npv(tmpdir, options):
    ...
    assert config.valid
    calculate = cli.CalculateNPV(config.snapshot, options.summary)
    calculate.run()
    calculate.write()
...
def test_extended_case_npv(tmpdir, options):
    ...
    calculate = cli.CalculateNPV(snapshot, options.summary)
    calculate.run()
    calculate.write()
...
def test_dates_outside_simulation_dates(tmpdir, options):
    ...
    calculate_outside_sim_dates = cli.CalculateNPV(snapshot, options.summary)
    ...
    calculate_inside_sim_dates = cli.CalculateNPV(snapshot, options.summary)
...
def test_keys_not_available(tmpdir, options):
    ...
    with pytest.raises(AttributeError) as excinfo:
        cli.CalculateNPV(snapshot, options.summary)
