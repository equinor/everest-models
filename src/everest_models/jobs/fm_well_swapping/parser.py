...
def build_argument_parser(lint: bool = False, **kwargs) -> Parser:
    ...
    parser, required_group = get_parser(
        description=(
            "Swaps well operation status over multiple time intervals according "
            "to multiple sets of priority values, status quota constraints and "
            "allowed status changing actions."
        )
    )
    ...
    parser.add_argument(
        *_CONSTRAINTS_ARGUMENT.split("/"),
        ...
        help="EVEREST-generated JSON file containing the values defining each swapping time interval values to be optimized",
    )
    parser.add_argument(
